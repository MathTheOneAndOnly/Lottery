...
contract Lotto is Ownable {
    ...
    mapping (uint256 => uint256[]) private excludedNumbers;
        ...
        if(currentRound == 0) {
            if(getTickets[currentGame].length == 0) {
                ...
                for(uint256 i = 0; i<amount; i++) {
                    uint256 rng = getRandom(getTickets[currentGame].length * 2, i);
    ...
    function finishRound() public {
        ...
        if (currentRound != 0 && _ticketsLeft <= 10) endGame();
            ...
            uint64 rng1 = getRandom(getTickets[currentGame].length, 5);
            uint64 rng2 = getRandom(getTickets[currentGame].length, 9);
    ...
    function roll() public {
        ...
        //gets 2 random numbers
        uint64 rng1 = getRandom(getTickets[currentGame].length, 1);
        uint64 rng2 = getRandom(getTickets[currentGame].length, 2);
    ...
    function _getBoundaries(uint256 game, uint256 startingRound, uint256 finishingRound) private view returns(uint256[4] memory boundaries){
        ...
        uint256 insideMin;
        uint256 insideMax;
        for(uint256 i = startingRound; i < finishingRound; i+=2) {
            uint256 _rng1 = excludedNumbers[game][i];
            uint256 _rng2 = excludedNumbers[game][i+1];
            uint256 _outside = _rng1 + getTickets[game].length - _rng2;
    ...
    /*              GET RANDOM             */
    /***************************************/
    function getRandom(uint256 max, uint256 nonce) public view returns (uint64) {
    ...
    /*            UI FUNCTIONS             */
    /***************************************/
    function ___GETTICKETSARRAY() external view returns(address[] memory) {
        return getTickets[currentGame];
    }

    function ___GETEXCLUDEDNUMBERS() external view returns(uint256[] memory) {
        return excludedNumbers[currentGame];
    }

    function ___GETROLLEDDATA() external view returns(uint64[4] memory) {
        return rolledNumbers[currentGame][msg.sender];
    }

    function ___GETMYTICKETSALIVE() external view returns (uint256[] memory) {
        uint256 count;
    ...
    function ___GETMYROLLABLETICKETS() external view returns (uint256[] memory) {
        ...
        if(_ticketsByUserLength == 0 || rolledNumbers[currentGame][msg.sender][0] == 1 || _canRoll == false) return rollableTickets;
            ...
            for(uint i; i < excludedNumbers[currentGame].length - 2; i+=2) {
                uint256 _rng1 = excludedNumbers[currentGame][i];
                uint256 _rng2 = excludedNumbers[currentGame][i+1];
                uint256 _outside = _rng1 + getTickets[currentGame].length-_rng2;
                uint256 _inside = _rng2 - _rng1;
                if(_outside > _inside){
                    if(_rng1 > outsideMin) outsideMin = _rng1;
                    if(_rng2 < outsideMax) outsideMax = _rng2;
            ...
            for(uint256 i; i < _ticketsByUserLength; i++) {
                uint256 ticket = getTicketsByUser[currentGame][msg.sender][i];
                if(ticket > outsideMin && ticket < outsideMax && ticket < insideMin && ticket > insideMax) {
                    rollableTickets[counter] = ticket;
                    counter++;
                }
            }
    ...
    function ___GETTICKETSBYOWNERARRAY() external view returns(uint256[] memory) {
        return getTicketsByUser[currentGame][msg.sender];
    }

    function ___GETTICKETSPOSITION(uint256 ticket) external view returns(uint256) {
